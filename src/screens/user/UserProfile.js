 ...
 * UserProfile.jsx
 *
 * Fix: getDoc (one-time) → onSnapshot (realtime listener)
 *      Profile now updates instantly when edited from EditProfile screen.
 */
...
export default function UserProfile({ navigation }) {
  ...
  // Track if entrance animation has already played (don't replay on realtime updates)
  ...
  // ── Realtime listener (replaces one-time getDoc) ──────────────────────────
  // onSnapshot fires immediately with cached/server data, then again on every
  // Firestore update — so EditProfile saves are reflected instantly.
  useEffect(() => {
    if (!uid) {
      setLoading(false);
      return;
  ...
  // Manual retry — just re-mount the listener by toggling a key,
  // simplest approach is to reset error and let the existing listener retry.
  const handleRetry = useCallback(() => {
    setLoadError(false);
    setLoading(true);
    // Re-trigger by remounting would need a key; instead just force re-read:
    const db     = getFirestore();
    const docRef = doc(db, 'users', uid);
  ...
  // ── Entrance animation — plays once after first load ──────────────────────
  useEffect(() => {
