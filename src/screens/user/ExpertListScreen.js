...
const ITEM_HEIGHT = 106; // padding 14*2 + avatar 56 + gap + tags ≈ 106

// ── Expert card — fully memoized ─────────────────────────────────────────────
...
export default function ExpertListScreen({ route, navigation }) {
  ...
  // ── Memoized sub-components passed as list props ──────────────────────────
  ...
  ), [search, handleSearch, handleClear, searchFocused,
      handleFocus, handleBlur, filtered.length]);
        ...
        >
          ...
          <View style={styles.headerSubRow}>
            ...
            <Text style={styles.headerSub}>
              {loading ? '…' : `${filtered.length} expert${filtered.length !== 1 ? 's' : ''} available`}ss
            </Text>
          </View>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  headerCenter : {
    flex: 1, alignItems: 'flex-start', marginTop: 4,marginLeft:12
  },
  ...
  headerPlaceholder : { width: 38 }, // mirrors backBtn width to keep title centred
