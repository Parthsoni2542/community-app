...
// //         {name?.senderName?.charAt(0)?.toUpperCase() || 'E'}
// //       </Text>
// //     </LinearGradient>
// //   </View>
// // ));
...
// //       ]}>
// //         {!isMe && (
// //           <ExpertAvatar name={item} palette={palette} invisible={isConsecutive} />
// //         )}
// //         {item.type === 'text' && (
...
// //       {isBroadcast ? 'Broadcast Chat' : 'Start the Conversation'}
// //     </Text>
// //     <Text style={styles.emptySubtitle}>
// //       {isBroadcast
// //         ? `Your message will be sent to all ${expertName} experts.\nThey typically respond within minutes.`
...
// //   },
// //   broadcastBadgeText: { fontSize: 10, fontWeight: '700', color: '#FFFFFF' },
...
// //         item.status === 'pending' ? (
// //           <Icon name="clock" size={12} color="rgba(191,219,254,0.9)" style={{ marginRight: 4 }} />
// //         ) : item.status === 'rejected' ? (
// //           <Icon name="x-circle" size={12} color="#FCA5A5" style={{ marginRight: 4 }} />
// //         ) : null
// //       )}
// //       <Text style={[styles.msgTime, isMe ? styles.msgTimeMe : styles.msgTimeOther]}>
// //         {time}
...
// //   // ── Scroll helper ─────────────────────────────────────────────────────────────
// //   // animated=false → instant jump (first open), animated=true → smooth scroll (new msg)
// //   const scrollToBottom = useCallback((animated = true) => {
// //     // Small timeout to let RN finish rendering the list items first
// //     setTimeout(() => {
...
// //           // Very first snapshot: hide loader then jump to bottom instantly
// //           setMessagesLoading(false);
// //           isFirstLoad.current = false;
// //           if (msgs.length > 0) {
// //             scrollToBottom(false); // no animation — instant jump
// //           }
// //         } else {
// //           // Subsequent updates (new messages arriving live)
// //           if (isNearBottom.current) {
// //             scrollToBottom(true); // smooth scroll only when already near bottom
// //           }
// //         }
// //       },
// //       () => {
...
// //       senderRole: 'user',
// //       status: 'pending',
// //       routedTo: expertId || null,  // ← admin yahan reroute kar sakta hai
// //       createdAt: serverTimestamp(),
// //       userName: profile?.name,
// //       senderName: profile?.name,
// //     };
...
// //         lastMessage: lastMsg,
// //         updatedAt: serverTimestamp(),
// //         lastMessageStatus: 'pending',  // ← admin dashboard ke liye
// //         userName: profile?.name,
// //         senderName: profile?.name,
// //         userId: uid,
// //       }),
...
// //           // ── When list fully lays out for first time, jump to bottom ──────
// //           onLayout={() => {
// //             if (isFirstLoad.current === false && messages.length > 0) {
// //               scrollToBottom(false);
// //             }
// //           }}
// //           // ── When content height changes (new message rendered) ────────────
// //           onContentSizeChange={() => {
// //             if (isNearBottom.current) {
// //               scrollToBottom(true);
// //             }
...
// //           windowSize={10}
// //           removeClippedSubviews={Platform.OS === 'android'}
// //           // ── This keeps existing messages in view when new ones load above ─
// //           maintainVisibleContentPosition={{ minIndexForVisible: 0 }}
// //         />
// //       )}
...
// // ─── StatusBadge ──────────────────────────────────────────────────────────────
// // Small inline indicator shown on the user's own messages
// const StatusBadge = React.memo(({ status }) => {
//   if (!status || status === MSG_STATUS.APPROVED) return null;

//   const configs = {
//     // [MSG_STATUS.PENDING]:  { icon: 'clock',         color: 'rgba(253,230,138,0.95)', label: 'Pending' },
//     // [MSG_STATUS.REJECTED]: { icon: 'x-circle',      color: 'rgba(252,165,165,0.95)', label: 'Rejected' },
...
// // ─── PendingOverlay ───────────────────────────────────────────────────────────
// // Dimmed overlay on bubble when message is pending approval
...
//         <View style={styles.msgMeta}>
//           {/* Status badge for user's own messages */}
...
//           {/* Tick icon: single clock for pending, double-check for approved */}
//           {isMe && (
//             // isPending
//             //   ? <MatIcon name="clock-outline"  size={13} color="rgba(253,230,138,0.85)" style={{ marginLeft: 3 }} />
//             //   : isRejected
//             //   ? <MatIcon name="close-circle"   size={13} color="rgba(252,165,165,0.85)" style={{ marginLeft: 3 }} />
//               // : 
//               <MatIcon name="check-all"      size={13} color="rgba(191,219,254,0.9)"  style={{ marginLeft: 3 }} />
//           )}
//         </View>
...
//           {isMe && (
//             isPending
//               ? <MatIcon name="clock-outline" size={11} color="rgba(253,230,138,0.9)" style={{ marginLeft: 2 }} />
//               : isRejected
//               ? <MatIcon name="close-circle"  size={11} color="rgba(252,165,165,0.9)" style={{ marginLeft: 2 }} />
//               : <MatIcon name="check-all"     size={11} color="rgba(255,255,255,0.9)" style={{ marginLeft: 2 }} />
//           )}
//         </View>
//       </View>
...
//               {isMe && (
//                 isPending
//                   ? <MatIcon name="clock-outline" size={11} color="rgba(253,230,138,0.85)" style={{ marginLeft: 2 }} />
//                   : isRejected
//                   ? <MatIcon name="close-circle"  size={11} color="rgba(252,165,165,0.85)" style={{ marginLeft: 2 }} />
//                   : <MatIcon name="check-all"     size={11} color="rgba(191,219,254,0.9)"  style={{ marginLeft: 2 }} />
//               )}
//             </View>
//           </View>
...
// // ─── ReroutedBanner ───────────────────────────────────────────────────────────
// // Shown below a rerouted message so user knows their question was forwarded
...
//             <VoiceBubble item={item} isMe={isMe} time={time} onPlay={onPlayVoice} />
//           )}
//           {/* Rerouted notice below the message */}
//           {/* {isMe && <ReroutedBanner item={item} />} */}
//         </View>
//       </View>
//     </>
//   );
...
// // ─── PendingInfoBar ───────────────────────────────────────────────────────────
// // Persistent top info strip: shows how many messages are awaiting admin review
...
//   // Count pending messages to show in info bar
...
//   // ── Firestore messages listener ──────────────────────────────────────────────
//   // User sees ALL their own messages (any status) + approved messages from others.
//   // Pending/rejected messages from experts are hidden from user (admin filters those).
//   useEffect(() => {
//     const db = getFirestore();
//     const q  = query(
//       collection(db, chatCollection, chatId, 'messages'),
...
//         // Filter rule:
//         //   - user's own messages → always show (any status)
//         //   - others' messages    → show only if approved (or no status set = legacy data)
//         const visibleMsgs = allMsgs.filter((m) => {
//           if (m.senderId === uid) return true;                          // own message
//           const s = m.status;
...
//   // ── Save message ─────────────────────────────────────────────────────────────
//   // Every user message starts as 'pending' — admin must approve / reroute / reject.
//   const saveMessage = useCallback(async (msgData) => {
//     const db = getFirestore();
...
//       status:          MSG_STATUS.PENDING,   // admin will change this
//       routedTo:        expertId || null,     // admin can change to reroute
...
//         senderName:        profile?.name,
//         userId:            uid,
//         pendingReview:     true,               // quick flag for admin queries
//       }),
//     ]);
...
//       {/* ── Pending-approval info strip ─────────────────────────────────────── */}
//       {/* <PendingInfoBar count={pendingCount} /> */}
...
//   // ── Bubble wrapper (holds bubble + reject reason) ──────────────────────────
...
//   bubbleMe:      { backgroundColor: '#0D7B7A', borderBottomRightRadius: 4 },
//   bubbleOther:   { backgroundColor: '#FFFFFF', borderBottomLeftRadius: 4, borderWidth: 1, borderColor: '#E2F4F4', shadowColor: '#0D7B7A', shadowOpacity: 0.06, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
//   // Pending/rejected visual variants for own messages
//   bubblePending: { backgroundColor: '#0F6B6A', opacity: 0.80 },   // slightly dimmed teal
//   bubbleRejected:{ backgroundColor: '#7F1D1D', opacity: 0.75 },   // dark red hint
...
//   // ── Status badge (inline next to time) ────────────────────────────────────
...
//   // ── Pending overlay (unused but available) ─────────────────────────────────
...
// Full-screen overlay used to view any image bubble at full size — works the
// same way for an image the user sent and for one received from an expert/admin.
...
export default function BrodcastChatScreen({ route, navigation }) {
  ...
  // Full-screen image viewer — holds the URL of whichever bubble image
  // (sent by me, or received from expert/admin) was tapped. null = closed.
  ...
  const saveMessage = useCallback(async (msgData) => {
    ...
    await Promise.all([
      ...
      updateDoc(doc(db, chatCollection, chatId), {
        ...
        userId: uid,
        pendingReview: false,
        ChatEnabled:true,               // quick flag for admin queries
      }),
    ]);
  }, [chatId, uid, profile, chatCollection, expertId]);
